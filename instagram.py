...
def get_following_list():
    try:
        ...
        time.sleep(3)
        allfoll=driver.find_element_by_xpath("//ul/li[3]/a/span").text.split(',')
        if len(allfoll)>1:
        ...
        print('Following: ',allfoll)
        time.sleep(2)
        driver.find_element_by_partial_link_text("following").click()
        time.sleep(3)
        last_height = driver.execute_script("return document.body.scrollHeight")
        dialog=driver.find_element_by_xpath("//div[@style='position: relative; z-index: 1;']/div/div[2]/div/div[2]")
